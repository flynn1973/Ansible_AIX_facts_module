...
options:
  name:
    description: Name of the fileset to install or update_all to update all filesets to latest level
    alias: filesets
    type: list
  state:
    description: state of the fileset or nimclient to be
    choices: [
              present,
              absent,
              installed,
    ...
    type: string
  lpp_source:
    description: Name of the lpp_source at the nimserver to be used for installation
    type: string
  spot:
    description: Name of the Spot at the nomserver
    type: string
...
def install(module):
    ...
    if list_filesets_to_install:
        ...
        cmd = "/usr/sbin/nimclient -o cust -a installp_flags=acgwXY -a lpp_source=" + \
...
def update(module):
    ...
    result = {}
    cmd = "/usr/sbin/nimclient -o cust -a installp_flags=acgwXY -a lpp_source=" + \
        module.params['lpp_source'] + " -a fixes=update_all"
    rc, out, err = module.run_command(cmd)
