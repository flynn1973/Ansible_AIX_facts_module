...
description:
    - installs and removes efixes deliverd by an nfs mount.
      If "name= ALL" is  given, it removes all efixes installed, which are not in the share, and install all efixes which are in the share, but not installed.
      If a single efix is given, this efix will be installed or removed appropiately.
version_added: "2.4"
options:
  name:
    description: name of the efix or ALL for all efixes in the share
    aliases: efix
    default: ALL
    type: list
  state:
    description: state of the efix
    choices: [ present,
               absent,
               installed,
               removed
    ...
    default: present
  nfs_server:
    description: name of the nfs server
    default: { NIM_MASTER }
  nfs_share:
    description: name of the remote directory, if not given, it will be "/export/nim/aix<OSVER>efix"
...
EXAMPLES = '''
# Synchronize ALL efixes from the NIM_MASTER in the standard share
...
def efixes_at_share(module, path):
    ...
    if efixlist == []:
        msg = "ERROR: No efixes found at share"
...
def main():
    ...
    # Install efix
        ...
        # findout which efixes are at the share
        ...
        if module.params['name'][0].upper() != 'ALL':
            ...
            for efix in module.params['name']:
                ...
                # if an efix is requested but it is not available at the share,
