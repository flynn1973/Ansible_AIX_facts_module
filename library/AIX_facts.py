...
ansible_facts: {
    ...
    "lparstat": [
        {
            "Active_CPUs_in_Pool": "23",
            "Active_Physical_CPUs_in_system": "23",
            "Capacity_Increment": "0.01",
	    "Desired_Capacity": "0.40",
	    "Desired_Memory": "8192",
	    "Desired_Variable_Capacity_Weight": "1",
	    "Desired_Virtual_CPUs": "4",
	    "Entitled_Capacity": "0.40",
	    "Entitled_Capacity_of_Pool": "895",
	    "Hypervisor_Page_Size": "-",
	    "Maximum_Capacity": "16.00",
	    "Maximum_Capacity_of_Pool": "2300",
	    "Maximum_Memory": "16384",
	    "Maximum_Physical_CPUs_in_system": "64",
	    "Maximum_Virtual_CPUs": "16",
...
def get_niminfo(module):
    ...
    try:
	    ...
	    '''
            niminfo = dict((k.strip(), v.strip(' "\n')) for k, v in (line.split(' ', 1)[1].split('=', 1) for line in ((l for l in open(file, 'r') if not l.startswith('#')))))
    except IOError as e:
...
def get_lparstat(module):
    ...
    if rc !=0:
        module.fail_json(msg="ERROR: could not complete lparstat -i", rc=rc, err=err)
    for line in out.splitlines():
...
def main():
    ...
    facts["lparstat"] = get_lparstat(module)
